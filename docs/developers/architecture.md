...
| `src/formatting.ts`     | Re-exports stream parsing utilities from [`@agentsy/core`](https://github.com/selfagency/agentsy)                         |
| `src/thinkingParser.ts` | Re-exports `ThinkingParser` from [`@agentsy/core`](https://github.com/selfagency/agentsy)                                 |
| `src/toolUtils.ts`      | Re-exports XML tool call utilities from [`@agentsy/core`](https://github.com/selfagency/agentsy); Ollama-specific helpers |
...
All LLM stream handling — XML context tag extraction, `<think>` block parsing, XML tool call extraction, and response formatting — is provided by [`@agentsy/core`](https://github.com/selfagency/agentsy). The source modules `src/formatting.ts`, `src/thinkingParser.ts`, and `src/toolUtils.ts` are thin re-export shims over the library's subpath exports.

See the [@agentsy/core API reference](https://github.com/selfagency/agentsy/tree/main/packages/core) for full documentation of the parsing primitives.
...
This is implemented via `splitLeadingXmlContextBlocks` and `dedupeXmlContextBlocksByTag` from `@agentsy/core/context`.
...
- **Thinking models** (`THINKING_MODEL_PATTERN`): models with `think`, `r1`, `kimi`, etc. in their name — `<think>...</think>` blocks are parsed via `ThinkingParser` from `@agentsy/core/thinking` and rendered as collapsible blockquotes before delivery
