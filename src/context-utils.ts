 ...
 */
export const OUTPUT_TOKEN_RESERVE = 512;
 ...
 */
export const SYSTEM_BUDGET_FRACTION = 0.4;
...
export function detectsRepetition(buffer: string, sensitivity: 'off' | 'conservative' | 'moderate'): boolean {
  if (sensitivity === 'off' || buffer.length === 0) {
    return false;
  }
  ...
  const base = 911_382_323;
  const prefix = new Uint32Array(n + 1);
  const pow = new Uint32Array(n + 1);
  pow[0] = 1;

  for (let i = 0; i < n; i++) {
    // biome-ignore lint/suspicious/noBitwiseOperators: intentional unsigned right shift for rolling hash
...
export function resolveContextLimit(modelReported: number, modelOptNumCtx?: number, settingMax?: number): number {
  if (modelOptNumCtx && modelOptNumCtx > 0) {
    return modelOptNumCtx;
  }
  if (modelReported > 0) {
    return modelReported;
  }
  if (settingMax && settingMax > 0) {
    return settingMax;
  }
