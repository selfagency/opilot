...
import {
  handleBuiltInOllamaConflict,
  logPerformanceSnapshot,
  setupChatParticipant,
} from './participantOrchestration.js';
...
export { handleBuiltInOllamaConflict, setupChatParticipant } from './participantOrchestration.js';
  ...
  try {
    for await (const chunk of response) {
      ...
      if (chunk.message?.tool_calls?.length) {
        for (const [tcIdx, toolCall] of chunk.message.tool_calls.entries()) {
