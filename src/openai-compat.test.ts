...
const asyncChunks = async function* (chunks: string[]): AsyncGenerator<string, void, void> {
...
describe('parseSseDataPayloadsFromTextChunks', () => {
  it('parses normal SSE data frames', async () => {
    const payloads = await collect(
      parseSseDataPayloadsFromTextChunks(asyncChunks(['data: {"a":1}\n\n', 'data: {"b":2}\n\n', 'data: [DONE]\n\n']))
    );
  ...
  it('ignores non-data lines and keeps only data payload', async () => {
    const payloads = await collect(
      parseSseDataPayloadsFromTextChunks(asyncChunks(['event: message\nid: 1\ndata: {"x":1}\n\n', 'data: [DONE]\n\n']))
    );
    expect(payloads).toEqual(['{"x":1}']);
  });

  it('handles trailing frame without final blank-line separator', async () => {
    const payloads = await collect(parseSseDataPayloadsFromTextChunks(asyncChunks(['data: {"final":true}'])));
    expect(payloads).toEqual(['{"final":true}']);
  });

  it('ignores trailing frame that is empty', async () => {
    const payloads = await collect(parseSseDataPayloadsFromTextChunks(asyncChunks(['data: {"a":1}\n\n', '   '])));
    expect(payloads).toEqual(['{"a":1}']);
  });
  ...
  it('ignores frames that have no data: lines', async () => {
    const payloads = await collect(
      parseSseDataPayloadsFromTextChunks(asyncChunks(['event: ping\n\n', 'data: {"b":2}\n\n', 'data: [DONE]\n\n']))
    );
  ...
  it('ignores frames where data payload is empty after trimming', async () => {
    const payloads = await collect(
      parseSseDataPayloadsFromTextChunks(asyncChunks(['data:   \n\n', 'data: {"c":3}\n\n', 'data: [DONE]\n\n']))
    );
...
describe('chatCompletionsOnce', () => {
  it('posts with stream=false and returns parsed JSON body', async () => {
    ...
    server.use(
      http.post('http://localhost:11434/v1/chat/completions', ({ request }) => {
