...
import type { ModelOptionOverrides, ModelSettingsStore } from './model-settings.js';
...
export function sanitizePatch(value: unknown): Partial<ModelOptionOverrides> {
  ...
  if (isFiniteNumber(candidate.temperature)) {
    patch.temperature = candidate.temperature;
  }
  if (isFiniteNumber(candidate.top_p)) {
    patch.top_p = candidate.top_p;
  }
  if (isFiniteNumber(candidate.top_k)) {
    patch.top_k = candidate.top_k;
  }
  if (isFiniteNumber(candidate.num_ctx)) {
    patch.num_ctx = candidate.num_ctx;
  }
  if (isFiniteNumber(candidate.num_predict)) {
    patch.num_predict = candidate.num_predict;
  }
  if (typeof candidate.think === 'boolean') {
    patch.think = candidate.think;
  }
  if (isFiniteNumber(candidate.think_budget)) {
    patch.think_budget = candidate.think_budget;
  }
...
export class ModelSettingsViewProvider implements vscode.WebviewViewProvider {
  ...
  private readonly options: ModelSettingsViewProviderOptions;

  constructor(options: ModelSettingsViewProviderOptions) {
