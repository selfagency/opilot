...
import type { reportError as ReportErrorFn } from './error-handler.js';
...
describe('reportError', () => {
  ...
  it('logs the error message via the provided logger', () => {
    const error = vi.fn();
    const logger = {
      info: vi.fn(),
      warn: vi.fn(),
  ...
  it('shows error dialog when showToUser: true', async () => {
    ...
    reportError(logger as any, 'Visible error', new Error('user-facing'), {
      showToUser: true
    });
